...
from .structures import Protein, Ligand
...
class RLADataset(Dataset):
    ...
    def _generate_graphs(self, metadata):
        ...
        try:
            ...
            name = metadata['name']
            ligid = name.split('_')[1]
            if ligid in ['UNK', 'rna', 'dna']: 
                print(f"[Warning] {name}: Skipping non-protein ligand {ligid}", file=sys.stderr)
                return None
            if self.glem_data_path:
                glems = self._load_glems(ligid)

                # if glems dict is empty, skip this sample
                if not glems:
                    print(f"[Warning] {name}: No GLEMS found for ligand {ligid}", file=sys.stderr)
                    return None
            else:
                glems = {}
            ...
            ligand_graph = self.ligand_featurizer.featurize_graph(ligand_obj, glems=glems, center=center)
    ...
    def _create_dummy_sample(self):
        ...
        for i in range(min(5, len(self.sample_metadata))):
            try:
                ...
                if sample and 'protein_virtual' in sample:
                    ref_graph = sample['protein_virtual']
