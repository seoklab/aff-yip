...
from ..structures import Protein, Ligand, VirtualNode 
from .utils import (
    stack_bb_coordinates, residue_dihedrals, sidechain_orientation, 
    backbone_orientation, _normalize_torch, rbf, positional_embeddings,
    stack_water_coordinates, pocket_virtual_nodes, water_embeddings, aa_one_hot
)
...
class ProteinFeaturizer:
    ...
    def featurize_graph_wo_virtual_nodes(
        self,
        protein_wo_water: Protein, 
        center=None,
        crop_size=None,
        rbf_D_count: int = 16, 
        positional_emb_dim: int = 16 
    ) -> Data:
        X_res_all, _ = stack_bb_coordinates(protein_wo_water) 
        sidechain_map = protein_wo_water.sidechain_map()
        # sidechain_map = sidechain_info(residue_s)
        # ca atoms
        ...
        node_s_all_residues = residue_dihedrals(X_res_all) # From original, uncropped full atom protein
        ...
        node_aa_features = aa_one_hot(protein_wo_water) # One-hot encoding of amino acids
        ...
        sidechain = sidechain_orientation(X_res_all) # (num_residues, 3)
        sidechain = sidechain.unsqueeze(-2) # (num_residues, 1, 3)
        backbone = backbone_orientation(X_res_all) # (num_residues, 2, 3)
        ...
        rbf_features = rbf(edge_dist, D_count=rbf_D_count, device=edge_index.device) if edge_dist.numel() > 0 else torch.empty((0,16))
        pos_emb_features = positional_embeddings(edge_index, num_embeddings=positional_emb_dim) if edge_index.numel() > 0 else torch.empty((0,16))
    ...
    def featurize_graph_with_virtual_nodes(
        ...
        if ligand_for_vn is not None:
            virtual_nodes_list = pocket_virtual_nodes(protein_wo_water, ligand_for_vn, only_backbone=True) # clash N,CA,C,O
        else:
            virtual_nodes_list = pocket_virtual_nodes(protein_wo_water, ligand, only_backbone=True) # clash N,CA,C,O
        ...
        num_scalar_features = node_s_res.size(1) # if node_s_res.numel() > 0 else 6 # Default from dihedrals
        if has_water and include_explicit_water:
            node_s_water = water_embeddings(X_water, num_embeddings=num_scalar_features)
